...
[KWin](https://userbase.kde.org/KWin/en) is the window manager used in KDE Plasma but increasingly also sees adoption on Qt-based desktop systems that are otherwise not using KDE Plasma, such as Nitrux, Deepin, CyberOS, PandaOS, CutefishOS, and helloSystem. However, KWin and its packaging are currently not optimized for use outside of KDE Plasma yet, drawing in "half of KDE Plasma" as dependencies.
